...
def lines_in_range(dir_, start, end, pattern='*.log'):
    ts_length = len(start)  # the timestamp length
    for log in sorted(glob(dir_ + '/' + pattern)):
        with open(log, 'r') as fd:
            for line in fd:
                timestamp = line[0:ts_length]
                if start <= timestamp < end:
                    yield line.strip()  # drop end-of-line character
for line in lines_in_range(dir_, start, end):
    print(line)
