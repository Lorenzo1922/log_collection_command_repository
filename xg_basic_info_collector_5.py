...
#data for log file filter
dir_ = '/log/'                        #path to the directory containing the logs
start = '2022-07-04 11:52:10'         #range start date/time
end = '2022-07-04 16:24:02'           #range end date/time
dt_start = '2022-07-04 11:52:10'
dt_end = '2022-07-04 16:24:02'
...
#tomcat log file which will include the extraction by time of the /log/tomcat.log
log_folder_xg = "/var/tslog/"
...
interact.send("mount -o remount, rw /")
interact.expect(prompt)
time.sleep(2)

interact.send("cd /var/tslog/")
interact.expect(prompt)
time.sleep(2)

interact.send("chmod -R 777 /var/tslog/")
interact.expect(prompt)
time.sleep(2)

#to extract a log file by a specific time range 
with open(external_path+'tomcat_filtered.log', 'w+') as myfile:               #file in which I will put the log_extraction
    with open('tomcat.log','r') as f:                  #original file
        for line in f:
...
interact.send("chmod -R 650 /var/tslog/")
interact.expect(prompt)
time.sleep(2)

interact.send("mount -o remount, ro /")
interact.expect(prompt)
time.sleep(2)
