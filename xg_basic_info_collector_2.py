...
ftp_port = 990
#access_id = input("Enter the access id without @eu2.apu.sophos.com: ")
ssh_username = input("Enter yourname,yourlastname.access id without @eu2.apu.sophos.com: ")
...
#create a file into the folder with the same name of the case id user enter before
...
devices = [
    {
        'ip_address' : 'eu2.apu.sophos.com',
        'port' : '2222',
        'vendor' : 'sophos',
        'username' : 'lorenzo.deluca,4bd1bed7-ae50-353b-a7b0-8456a8420a90',   #Lorenzo Test Machine 4bd1bed7-ae50-353b-a7b0-8456a8420a90
...
for device in devices:
    ...
    if device['vendor'] == 'sophos':
        
        #Disabled invoke_shell as we won't need it anymore with paramiko_expect
        #interact = ssh_client.invoke_shell()

        #Bypasses the select menu on SFOS
        interact.send("5")
        ...
        interact.send("nvram get '#li.serial'")
        interact.expect(prompt)
        
        #Ignore for now
        #cmd_output_nvram = interact.current_output_clean
        #time.sleep(2)
        #interact.send("cish -c system diagnostics show version-info")
        #interact.expect(prompt)
        #cmd_output_cish = interact.current_output_clean
        #interact.send("""opcode ctr -ds nosync -t json -b '{"problemdesc":"debugging purpose","logs":"1","systemsnap":"0"}'""")
        #interact.expect(prompt, timeout=None)
        #cmd_output_ctr = interact.current_output_clean
        interact.send("ls -lahs /sdisk/ctrfinal/")
        interact.expect(prompt)
        ...
        #Download file from remotehost to localhost
        ...
        #Close ssh session as we wouldn't need it anymore
        ssh_client.close


#Ignore for now
#send file to sophos ftp server
#command syntax
#curl --ftp-ssl ftp://ftp.sophos.com:990 -u <username>:<password> -v -T {/path/file}
#connect to the FTP server

#Working FTP session to upload files from laptop to our FTP server
#ftp = ftplib.FTP_TLS()
...
#    ftp.storbinary(f"STOR {external_filename}", file)
#    # Get list of files
#    ftp.dir()
